...
describe('create testerino', function() {
	...
	it('create user', function() {
		...
		cy.wait(500)
		cy.get("#user-form [name=givenname]").clear().type("Max")
		cy.get("#user-form [name=sn]").clear().type("Musterman")
		cy.get("#user-form [name=initials]").clear().type("MM")
		cy.get("#user-form [name=o]").clear().type("Max Musterman's Farb Tapeten Königreich")
		cy.get("#user-form [name=title]").clear().type("Tapetenmeister")
		
		cy.get(".nav-tabs .nav-item").eq(2).find("a").click()
		cy.wait(500)
	...
	it('login as user and change stuff', function() {
		...
		cy.wait(2500)
		cy.get("#user-form [name=initials]").clear().type("MMMMMMMM")
		cy.get("#user-form [name=o]").clear().type("MAXimal Ideenlose Org.")
		cy.get("#user-form [name=title]").clear().type("Nothing, just nothing")
		cy.get("#user-form input.submit").click()
	})
	it('login again and check infos', function() {
		...
		cy.get("#userlist .selectable").first().click()
		cy.wait(500)
		cy.get("#user-form [name=initials]").should('has.value', "MMMMMMMM")
		cy.get("#user-form [name=o]").should("has.value","MAXimal Ideenlose Org.")
		cy.get("#user-form [name=title]").should("has.value","Nothing, just nothing")
		cy.get('#topmenu .logout').should("be.visible").click()
